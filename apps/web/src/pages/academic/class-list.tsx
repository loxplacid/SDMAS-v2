...
import type { ClassResponse, ClassCreate, ClassUpdate, AcademicYearResponse } from '../../api/generated/types'
...
type ClassFormData = { name: string; academic_year_id: number | null; status?: string | null }
...
const columns = [
  { key: 'name', header: 'Name' },
  {
    ...
    render: (c: ClassResponse) => `Year #${c.academic_year_id}`,
  },
  {
    key: 'status', header: 'Status',
    render: (c: ClassResponse) => <Badge variant={statusBadge[c.status] || 'default'}>{capitalize(c.status)}</Badge>,
  },
]
...
export function ClassListPage() {
  ...
  const [formData, setFormData] = useState<ClassFormData>({ name: '', academic_year_id: null })
  ...
  const openCreateModal = () => {
    setEditing(null)
    setFormData({ name: '', academic_year_id: yearFilter ? Number(yearFilter) : null })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-gray-900">Classes</h1><p className="text-gray-500 mt-1">{total} class{total !== 1 ? 'es' : ''}</p></div>
      ...
      <Card>
        {loading ? <Loading text="Loading classes..." /> : error ? <ErrorState message={error} onRetry={() => fetch({ page, size, academic_year_id: yearFilter ? Number(yearFilter) : undefined, status: statusFilter || undefined })} /> : (
          <>
            <Table columns={[...columns, { key: 'actions', header: 'Actions', render: (c: ClassResponse) => (<div className="flex gap-2" onClick={(e) => e.stopPropagation()}><Button variant="ghost" size="sm" onClick={() => openEditModal(c)}>Edit</Button></div>) }]} data={data} keyExtractor={(c) => c.id} emptyMessage="No classes found." onRowClick={(c) => navigate(`/academic/classes/${c.id}`)} />
      ...
      >
        ...
        <Form onSubmit={handleSubmit}>
          ...
          <Select label="Academic Year" value={String(formData.academic_year_id ?? '')} onChange={(e) => setFormData({ ...formData, academic_year_id: e.target.value ? Number(e.target.value) : null })} error={formErrors.academic_year_id}
            options={years.map((y) => ({ value: String(y.id), label: y.name }))} placeholder="Select academic year" />
          {editing && (
