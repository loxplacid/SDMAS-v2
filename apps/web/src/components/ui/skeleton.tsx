...
export function Skeleton({ className, variant = 'text', width, height, count = 1 }: SkeletonProps) {
  const baseClass = 'animate-skeleton rounded-lg bg-[var(--color-border-light)]'
  const variants = {
    text: 'h-4 w-full',
...
export function TableSkeleton({ rows = 5, cols = 4 }: { rows?: number; cols?: number }) {
  return (
    <div className="space-y-4">
      <div className="flex gap-5 pb-3 border-b border-[var(--color-divider)]">
      ...
      {Array.from({ length: rows }, (_, rowIdx) => (
        <div key={rowIdx} className="flex gap-5">
          {Array.from({ length: cols }, (_, colIdx) => (
            <Skeleton key={`r${rowIdx}-c${colIdx}`} className="h-3 flex-1" />
          ))}
        </div>
      ))}
    </div>
...
export function FormSkeleton({ fields = 4 }: { fields?: number }) {
  return (
    <div className="space-y-5 animate-fade-in">
      <div className="space-y-3"><Skeleton className="h-5 w-48" /><Skeleton className="h-3 w-64" /></div>
