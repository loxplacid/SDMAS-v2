...
- the ``causation_id`` (the event id that caused the current one) so the
  causal chain is traceable end-to-end
...
_causation_var: contextvars.ContextVar[str | None] = contextvars.ContextVar(
    "events_causation_id", default=None
)
...
def get_causation_id() -> str | None:
    """Return the current causation id (the event id that caused the
    current operation), or None when unset."""
    return _causation_var.get()
...
def event_context(
    *,
    ...
    if causation_id is not None:
        tokens.append((_causation_var, _causation_var.set(causation_id)))
    try:
        yield
    finally:
