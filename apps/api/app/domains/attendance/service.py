...
class AttendanceService:
    ...
    async def record_daily_attendance(
        ...
        for item in data.records:
            ...
            student = await self._validate_student(item.student_id)
            await self._validate_student_enrolled_in_section(
                student.id, data.section_id
            )

            duplicate = await self.repo.find_duplicate(
                student.id, data.attendance_date, data.section_id
            )
            if duplicate is not None:
                raise ConflictError(
                    f"Attendance record already exists for student {student.id} on {data.attendance_date} in section {data.section_id}"
                )
